	...
	return (
		<div className="relative bg-[#020812] rounded-xl p-4 border border-violet-500/25 font-mono text-xs overflow-hidden">
			...
			<div className="text-violet-400 mb-3">
				▸ TLSNotary / Reclaim Protocol — ZK Proof Generator
			</div>
			<div className="space-y-2">
				{stages.map((s, i) => {
					...
					const isVerified =
						i === stages.length - 1 && stage === stages.length - 1;
					return (
						<div
							key={i}
							className={cn(
			...
			{stage === stages.length - 1 && (
				<div className="mt-3 p-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-300">
					Proof ID: <span className="text-emerald-200">0x7fa3c8…d901e2</span> ·
					Arc Block #4,129,841
				</div>
			)}
		</div>
...
function ChatWindow({ onDispute }: { onDispute: () => void }) {
	...
	const [msgs, setMsgs] = useState<ChatMessage[]>([
		{
			from: "system",
			text: "Escrow channel opened. Both parties are connected.",
			time: "14:02",
		},
		{
			from: "buyer",
			text: "I've completed the ₦500,000 transfer to your GTBank account ending ****7812.",
			time: "14:05",
		},
		{
			from: "seller",
			text: "Checking my account now — please hold.",
			time: "14:06",
		},
		{
			from: "seller",
			text: "I can see the credit. Releasing USDC now.",
			time: "14:09",
		},
		{
			from: "system",
			text: "⚠️ Cryptographic proof of payment received and verified by Arc Oracle. Dispute window: 15 mins.",
			time: "14:09",
		},
	]);
	...
	return (
		<div className="flex flex-col gap-3 step-content">
				...
				{msgs.map((m, i) => (
					...
					>
						{m.from === "system" ? (
							<div
								className={cn(
									"text-[10px] px-3 py-2 rounded-xl max-w-[90%] text-center leading-relaxed",
									m.danger
							...
							<div
								className={cn(
									...
									m.from === "buyer" ? "items-end" : "items-start",
								)}
							>
								<span className="text-[10px] text-slate-500 px-1">
									{m.from === "buyer" ? "You (Buyer)" : "Seller · OluwaseunAd"}
								</span>
								<div
									className={cn(
			...
			<div className="flex gap-2">
				...
				<button
					onClick={send}
					className="w-8 h-8 rounded-xl bg-violet-600 hover:bg-violet-500 flex items-center justify-center transition-all shrink-0"
				>
			...
			{!disputed ? (
				...
				<div className="p-3 rounded-xl bg-red-500/8 border border-red-500/25 text-xs text-red-300 leading-relaxed">
					...
					<div className="text-red-400/70">
						...
						locked from the accused party. An Arc DAO arbitrator will review
						within 2 hours. Filing false disputes incurs a permanent reputation
						slash.
					</div>
				</div>
			)}
			...
			<div className="flex items-center justify-between mb-5 ">
				<div>
					...
					<div className="text-[10px] text-slate-500 pl-8">
						Powered by Arc Network · Gas in USDC
					</div>
				</div>
				...
				<div className="step-content space-y-4">
					...
					<div className="p-3 rounded-xl bg-violet-500/8 border border-violet-500/20 flex items-start gap-2">
						...
						<div className="text-xs text-slate-400 leading-relaxed">
							...
							<span className="font-mono text-white">0.5%</span> ({platformFee}{" "}
							USDC).
						</div>
					</div>
			...
			{step === 1 && (
				<div className="step-content space-y-4">
					<div className="p-3 rounded-xl bg-amber-500/8 border border-amber-500/25 flex items-start gap-2">
						...
						<div className="text-xs text-amber-300/80 leading-relaxed">
							<span className="text-amber-300 font-semibold">
								Visual receipts are deprecated.
							</span>{" "}
							Screenshot-based proofs are rejected to prevent fraud via doctored
							images. Choose a cryptographic verification method.
						</div>
					</div>
						...
						<div className="grid grid-cols-2 gap-3">
							...
							>
								<Fingerprint
									size={18}
									className={
										verifyMode === "zk" ? "text-violet-400" : "text-slate-500"
									}
								/>
								<div className="text-xs font-semibold text-white mt-2">
									ZK Web-Proof
					...
					{verifyMode === "bank" && !bankLinked && (
						<div className="space-y-2">
							<p className="text-xs text-slate-400">
								Connect your bank to auto-verify this transfer via open banking
								oracle.
							</p>
						...
						<div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center gap-3">
							...
							<div>
								<div className="text-sm text-emerald-300 font-semibold">
									Bank Account Linked: OPay (****1234)
								</div>
								<div className="text-[10px] text-emerald-400/70 mt-0.5">
									Transaction of ₦500,000 auto-confirmed via Mono oracle at
									14:09 WAT
								</div>
							</div>
						</div>
					...
					{verifyMode === "zk" && !zkDone && (
						<ZKProofPanel onComplete={handleZkComplete} />
					)}
					{verifyMode === "zk" && zkDone && (
						<div className="p-3 rounded-xl bg-violet-500/10 border border-violet-500/30 flex items-center gap-3">
							...
							<div>
								...
								<div className="text-[10px] text-violet-400/70 mt-0.5 font-mono">
									Arc Block #4,129,841 · Proof: 0x7fa3c8…d901e2
								</div>
							</div>
						</div>
					)}
					...
					<div className="flex gap-2">
						<button
							onClick={() => setStep(0)}
							className="flex-1 py-2.5 rounded-xl border border-slate-700 text-slate-400 text-xs hover:bg-slate-800 transition-all"
						>
							← Back
						</button>
						<button
			...
			{step === 2 && (
				<div className="step-content space-y-4">
					...
					<p className="text-center text-[10px] text-slate-500 -mt-2">
						Seller must release USDC within this window
					</p>

					<div className="space-y-2">
						{[
							{
								label: "Seller USDC Locked",
								value: `${usdcAmount} USDC`,
								color: "green" as const,
								icon: <Lock size={11} className="text-emerald-400" />,
							},
							{
								label: "Buyer Collateral Deposited (Anti-Fraud Lock)",
								value: "25 USDC",
								color: "amber" as const,
								icon: <Shield size={11} className="text-amber-400" />,
							},
							{
								label: "Arc Escrow Status",
								value: "Secured",
								color: "violet" as const,
								icon: <CheckCircle size={11} className="text-violet-400" />,
							},
							{
								label: "Payment Verification",
								value: bankLinked
									? "OPay Oracle Confirmed"
									: "ZK Proof On-Chain",
								color: "green" as const,
								icon: <Check size={11} className="text-emerald-400" />,
							},
			...
			{step === 3 && (
				<div className="step-content space-y-3">
					...
					{!disputeActive && (
						<button
							onClick={() => setStep(2)}
							className="w-full py-2 rounded-xl border border-slate-700 text-slate-400 text-xs hover:bg-slate-800 transition-all"
						>
							← Back to Escrow
						</button>
					)}
