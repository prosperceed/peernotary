  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="text-sm font-mono text-emerald-300">{MOCK_ADDRESS}</span>
        <ChevronDown
          size={13}
          className={cn(
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-56 rounded-2xl border border-slate-700/60 bg-[#0D1322]/95 backdrop-blur-xl shadow-2xl shadow-black/50 overflow-hidden z-50">
          <div className="p-3 border-b border-slate-800/60">
            <div className="text-[10px] text-slate-500 mb-1">Connected wallet</div>
            <div className="font-mono text-xs text-slate-300 truncate">{MOCK_ADDRESS}</div>
          ...
          <div className="p-2 space-y-0.5">
            ...
            <button
              onClick={() => { setConnected(false); setOpen(false); }}
...
// ─── GLOBAL NAV ───────────────────────────────────────────────────────────────
export default function GlobalNav({ currentView, setCurrentView }: GlobalNavProps) {
    ...
    >
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          >
            ...
            <span className="font-bold text-white text-sm">
              Arc<span className="text-violet-400">P2P</span>
            </span>
          </button>
          <LiveTicker />
        </div>
      ...
      <div className="md:hidden flex items-center gap-1 px-4 pb-2.5 border-t border-slate-800/40 pt-2">
        {navLinks.map(({ label, view }) => (
          ...
          >
            {view === "trade" ? "P2P Trade" : view === "dev" ? "Dev Portal" : label}
          </button>
        ))}
      </div>
    </header>
